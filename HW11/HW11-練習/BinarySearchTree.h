...
{
	//使得重複的word加入search tree時，不會產生新的節點，而是記錄該word已被加入幾次。
	...
	else if(subTreePtr->getItem() == newNodePtr->getItem()){ //若[現在節點值] 相同於 [新添加節點值]
		subTreePtr->setCount(subTreePtr->getCount()+1); //then [現在節點值]+=1
	}
	else {
...
BinaryNode<ItemType>* BinarySearchTree<ItemType>::removeValue(BinaryNode<ItemType>* subTreePtr,
                                     ...
                                     bool& success)
{
	//使得刪除search tree中的某word時，除非 count是1，
	//否則不移除該節點，而僅是減少count記錄的次數。
	...
	if ( subTreePtr->getItem() == target ) { //找到待刪除目標
		if(subTreePtr->getCount() > 1)
			subTreePtr->setCount(subTreePtr->getCount()-1); //若[現在節點值]多於1，就單純-1而不刪除節點
		else 
